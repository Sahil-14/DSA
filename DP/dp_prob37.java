 ...
 * 
 * 
 * Find maximum subsquare in a matrix made up of Xs and Os such that all four
 ...
 * Once this matrix is build look for biggest subsquare by getting min of hori
 * and ver at each point and checking
...
public class dp_prob37 {
  ...
  public int findSubSquare(char input[][]) {
    ...
    // start iterating from bottom right corner and find min of hori or ver at every
    // cell.
    // If this is greater than 1 then see if you can find a number between this min
    // and 1
    // such that on left's ver and top's hori is greater greater than or equal to k.
    int max = 1;
    for (int i = T.length - 1; i >= 0; i--) {
      for (int j = T[0].length - 1; j >= 0; j--) {
        if (T[i][j].ver == 0 || T[i][j].ver == 1 || T[i][j].hor == 1) {
          continue;
        }
        int min = Math.min(T[i][j].ver, T[i][j].hor);
        int k = 0;
        for (k = min; k > 1; k--) {
