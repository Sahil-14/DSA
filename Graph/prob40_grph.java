 ...
 * You are given an m liter jug and a n liter jug. Both the jugs are initially
 * empty. The jugs don’t have markings to allow measuring smaller quantities.
 * You have to use the jugs to measure d liters of water where d is less than n.
 * 
 * 
 * (X, Y) corresponds to a state where X refers to the amount of water in Jug1
 * and Y refers to the amount of water in Jug2
 * Determine the path from the initial state (xi, yi) to the final state (xf,
 * yf), where (xi, yi) is (0, 0) which indicates both Jugs are initially empty
 * and (xf, yf) indicates a state which could be (0, d) or (d, 0).
 * 
 ...
 * full, (X, Y) -> (X-d, Y+d
 * 
 * We start at an initial state in the queue where both the jugs are empty. We
 * then continue to explore all the possible intermediate states derived from
 * the current jug state using the operations provided.
 * 
 * We also, maintain a visited matrix of states so that we avoid revisiting the
 * same state of jugs again and again
 * see the states here
